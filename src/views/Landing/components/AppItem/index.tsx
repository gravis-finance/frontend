  ...
  return (
    <Container alignItems="center" flexDirection="column" p="3rem" justifyContent="space-between" {...props}>
      <Flex alignItems="center" flexDirection="column">
        <Flex alignItems="center" flexDirection={{ _: 'row', md: 'column' }} gridGap={{ _: '1.5rem', md: '2rem' }}>
          <IconWrapper justifyContent="center" backgroundColor={backgroundColor}>
            <Icon size={{ _: '5rem', md: '8rem' }} />
          </IconWrapper>
          <Text fontSize="2.4rem" style={{ fontWeight: 800, letterSpacing: '-0.02em' }}>
            {title}
          </Text>
        </Flex>
        <Text style={{ textAlign: 'center', letterSpacing: '0px' }} fontSize="1.4rem" mt={{ _: '1rem', md: '0.3rem' }}>
          {description.split('\n').map((str) => (
            <p key={str}>{str}</p>
          ))}
        </Text>
      </Flex>
      <Button width="100%" as="a" href={href} target="_blank" mt={{ _: '2.5rem', md: 0 }}>
        Open {title}
      </Button>
    </Container>
  )
